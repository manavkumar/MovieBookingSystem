...
public interface ShowRepository extends JpaRepository<Show, Long> {
    @Query("SELECT s FROM Show s WHERE s.theatre.city = :city AND DATE(s.showTime) = DATE(:date) AND (:movieId IS NULL OR s.movie.id = :movieId) AND (:theatreId IS NULL OR s.theatre.id = :theatreId)")
